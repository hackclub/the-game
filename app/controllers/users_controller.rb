class UsersController < ApplicationController
  ...
  def show
    ...
    purchase_items = user.purchases.map { |purchase| { name: "Purchased #{"#{purchase.quantity}x" if purchase.quantity > 1} \"#{purchase.item.name}\" for #{pluralize(purchase.item.price * purchase.quantity, "ticket")}", date: purchase.created_at, link: current_user.admin? ? order_path(purchase) : orders_path } }
