...
export default function Stats({
  ...
  const funnelData = [
    ...
    { name: "Project created", value: stats.user_project_created_count, fill: "#facc15" },
    { name: "Project submitted", value: stats.user_project_submitted_count, fill: "#fb923c" },
    { name: "Project approved", value: stats.user_project_shipped_count, fill: "#f87171" },
  ];
  ...
  return (
    <Layout>
      <div className="mb-6">
        ...
        <p className="italic text-gray-500">As of {new Date(stats.date).toLocaleString()}</p>
      </div>
      ...
      <div className="flex gap-4 flex-wrap">
        {[
          ...
          ...(invite_projected_count !== null ? [{ label: "Projected invites by May 22", value: invite_projected_count }] : []),
        ].map(({ label, value }) => (
          <div key={label} className="border border-gray-200 rounded-lg p-5 min-w-40 flex flex-col gap-1">
              ...
              {funnelData.map((row, i) => {
                ...
                return (
                  <tr key={row.name} className="border-b border-gray-100">
                    <td className="pr-4 py-1 font-medium w-40">{row.name}</td>
                    <td className="pr-4 py-1 text-right tabular-nums w-20">{row.value.toLocaleString()}</td>
                    <td className="pr-4 py-1 text-gray-500 w-24">{pct}% of all</td>
                    {prevPct !== null && (
                      <td className="py-1 text-gray-400 text-xs">{prevPct}% of prev</td>
                    )}
                  </tr>
                );
              })}
      ...
      {invite_orders_over_time.length > 0 ? (
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={invite_orders_over_time} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="date" tickFormatter={formatDate} tick={{ fontSize: 11 }} interval="preserveStartEnd" />
            <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
            <Tooltip labelFormatter={(v) => formatDate(String(v))} formatter={(v) => [v, "Orders"]} />
            <Bar dataKey="count" fill="#6366f1" radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      ) : (
        <p className="text-gray-400 text-sm">No invite order data.</p>
      )}

      <SectionHeading>Shop orders over time</SectionHeading>
      <p className="text-sm text-gray-500 mb-3">Pending and on-hold orders only — fulfilled orders excluded.</p>
      {orders_over_time.length > 0 ? (
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={orders_over_time} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="date" tickFormatter={formatDate} tick={{ fontSize: 11 }} interval="preserveStartEnd" />
            <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
            <Tooltip labelFormatter={(v) => formatDate(String(v))} formatter={(v) => [v, "Orders"]} />
        ...
        <p className="text-gray-400 text-sm">No order data.</p>
      )}
    </Layout>
  );
}
